// 1
{
  const sumAllNumbers = (...args: number[][]) => {
...
// 2
{
  const doubleValue: (inputVal: unknown) => void = (inputVal) => {
    if (typeof inputVal === "number") console.log(inputVal * 2);
    else if (typeof inputVal === "string") console.log(inputVal.length * 2);
  };
  doubleValue(12);
  doubleValue("hello");
}
...
// 5
{
  const formatInfo: (inputObj: { name?: string; age?: number }) => void = (
    inputObj
  ) => {
    if (inputObj.name) {
...
// 9
{
  const compareValues: (str: string, num: number) => string = (str, num) => {
